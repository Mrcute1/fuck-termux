...
# Coded By B
ALI RAZA
# My Whatsall ( Main Ni Bataon Ga Mughy Apna Mazak Ni Banwana Hai😂😂)
...
def moch_yayan():
    ...
    print ' (\033[0;96m•\033[0m) NAWT : %s'%(nama);time.sleep(0.03)
    print ' (\033[0;96m•\033[0m) IP TO   : %s'%(IP)
    print '___________________________________________________________\n';time.sleep(0.03)
    ...
    if pepek == '':
        ...
        try:
            ...
            print("\n %s[%s×%s] BA BATALE JEY MAHELA.!("%(N,M,N))
            raw_input('\n  [ %sENTER%s ] '%(O,N));moch_yayan()
    elif pepek in['8','08']:
        seting_yntkts()
    elif pepek in['9','09']:
        ...
        print '\n %s[%s×%s] menu [%s%s%s] Back To Main Menu bro!'%(N,M,N,M,pepek,N);time.sleep(2);moch_yayan()
...
def teman(kontol):
    ...
    try:
        ...
        xxx.close()
        jalan('\n\n %s[%s✓%s] Crack File Saved'%(N,H,N))
        print ' [%s•%s] COPY BKA.! ( %s%s%s )'%(O,N,M,cin,N)
        print 50 * '-'
...
def publik(kontol):
    ...
    try:
        ...
        xxx.close()
        jalan('\n\n %s[%s✓%s] Crack ID File Saved'%(N,H,N))
        print ' [%s•%s] COPY BKA.! ( %s%s%s )'%(O,N,M,knt,N)
        print 50 * '-'
        raw_input(' [%s ENTER%s ] '%(O,N));moch_yayan()
    except (KeyError,IOError):
        os.remove(knt)
...
def followers(kontol):
    ...
    try:
        ...
        xxx.close()
        jalan('\n\n %s[%s✓%s] Crack File Saved total followers'%(N,H,N))
        print ' [%s•%s] COPY BKA.! ( %s%s%s )'%(O,N,M,ah,N)
        print 50 * '-'
        raw_input(' [%s ENTER%s ] '%(O,N));moch_yayan()
    except (KeyError,IOError):
        os.remove(ah)
...
def postingan(kontol):
    ...
    try:
        ...
        xxx.close()
        jalan('\n\n %s[%s✓%s] Crack ID File Saved'%(N,H,N))
        print ' [%s•%s] COPY BKA ( %s%s%s )'%(O,N,M,ahh,N)
        print 50 * '-'
        raw_input(' [%s ENTER%s ] '%(O,N));moch_yayan()
    except (KeyError,IOError):
        os.remove(ahh)
...
def cek_ingfo(kontol):
    ...
    jalan('\n [%s✓%s] successfully checked data² from facebook account\n\n'%(O,N));exit()
...
def seting_yntkts():
    ...
    ytbjts = raw_input('\n %s[%s?%s] choose : '%(N,O,N))
    if ytbjts == '':
        print '\n %s[%s×%s] Fill Empty Place Kentod'%(N,M,N);time.sleep(2);seting_yntkts()
    elif ytbjts in['1','01']:
...
def yo_ndak_tau_ko_tanya_saia():
    ...
    if _asu_ == '':
        print '\n %s[%s×%s] Fill Empty Place Kentod'%(N,M,N);yo_ndak_tau_ko_tanya_saia()
    elif _asu_ in['Y','y']:
        ...
        jalan('\n %s[%s✓%s] successfully using your hp user agent...'%(N,H,N))
        raw_input('\n  %s[ %skembali%s ]'%(N,O,N));moch_yayan()
    elif _asu_ in['T','t']:
        _agen_ = raw_input(' [%s?%s] Enter user agent :%s '%(O,N,H))
        open('YNTKTS.txt', 'w').write(_agen_);time.sleep(2)
        jalan('\n %s[%s✓%s] successfully replaced user agent...'%(N,H,N))
...
class __crack__:
    ...
    def plerr(self):
        ...
        if ___yayanganteng___ in ('Y', 'y'):
            ...
            while True:
                ...
                if pwek == '':
                    print '\n %s[%s×%s] Fill EmPty passwors box'%(N,M,N)
                elif len(pwek)<=5:
                    print '\n %s[%s×%s] Enter minimum 6 digit password'%(N,M,N)
                else:
                    def __yan__(ysc=None): # ycs => Pakistan 786786 123456:3
                        cin = raw_input('\n [*] method : ')
                        if cin == '':
                            print '\n %s[%s×%s] Fill Every Blank Place'%(N,M,N);__yan__()()
                        elif cin == '1':
                            ...
                            print '\n %s[%s×%s] correct input'%(N,M,N);__yan__()
                    print '\n [ Choose method login - 1 option is best² ]\n'
            ...
            print '\n %s[%s×%s] Y/t stupid!'%(N,M,N);self.plerr()
        return
    ...
    def __pler__(self):
        yan = raw_input('\n [*] method : ')
        if yan == '':
            print '\n %s[%s×%s] Fill Blank Place'%(N,M,N);self.__pler__()
        elif yan in ('1', '01'):
            print '\n [%s+%s] Ok Result Saved To -> results/OK-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print ' [%s+%s] Cp result saved to -> results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
            ...
            print '\n %s[%s×%s] Input Correct'%(N,M,N);self.__pler__()
